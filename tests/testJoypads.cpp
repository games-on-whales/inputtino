    ...
    while (SDL_PollEvent(&event) != 0) {
        switch (event.type) {
            case SDL_CONTROLLERDEVICEADDED:
                std::cout << "SDL_CONTROLLERDEVICEADDED " << SDL_GameControllerNameForIndex(event.cdevice.which)
                          << std::endl;
                break;
            ...
            case SDL_CONTROLLERDEVICEREMAPPED:
                std::cout << "SDL_CONTROLLERDEVICEREMAPPED " << SDL_GameControllerNameForIndex(event.cdevice.which)
                          << std::endl;
                break;
        }
    }
...
TEST_CASE_METHOD(SDLTestsFixture, "PS Joypad", "[SDL]") {
    ...
    auto joypad_ptr = *Joypad::create(Joypad::PS, Joypad::RUMBLE | Joypad::ANALOG_TRIGGERS);
...
TEST_CASE_METHOD(SDLTestsFixture, "XBOX Joypad", "[SDL]") {
    // Create the controller
    auto joypad_ptr = *Joypad::create(Joypad::XBOX, Joypad::RUMBLE | Joypad::ANALOG_TRIGGERS);
    auto joypad = *joypad_ptr;
...
TEST_CASE_METHOD(SDLTestsFixture, "Nintendo Joypad", "[SDL]") {
    // Create the controller
    auto joypad_ptr = *Joypad::create(Joypad::NINTENDO, Joypad::RUMBLE);
    auto joypad = *joypad_ptr;
